...
export default function TournamentStats8({ teams, matches }: Props) {
  const playedMatches = matches.filter(m => m.status === 'completed').length;
  const liveMatches   = matches.filter(m => m.status === 'live').length;
  const totalMatches  = matches.length;

  const activeTeams = teams.filter(team =>
    matches.some(m => m.status !== 'completed' && (m.team1_id === team.id || m.team2_id === team.id))
  );
  const eliminatedTeams = teams.filter(t => !activeTeams.some(at => at.id === t.id));

  const finalMatch = matches.find(m => m.id === 'M16');
        ...
        {activeTeams.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {activeTeams.slice(0, 4).map(team => (
              <span key={team.id} className="px-2 py-0.5 text-xs rounded bg-gray-800 text-gray-300">{team.name}</span>
            ))}
            {activeTeams.length > 4 && (
              <span className="px-2 py-0.5 text-xs rounded bg-gray-800 text-gray-500">+{activeTeams.length - 4}</span>
            )}
          </div>
        )}
      </div>
        ...
        {champion ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {nextMatch ? (
                <div className="text-sm font-bold text-white">{nextMatch.id}</div>
              ) : (
