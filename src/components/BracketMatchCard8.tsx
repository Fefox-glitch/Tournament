...
import { MATCH_W8, MATCH_H8 } from '../data/bracketLayout8';

interface Props {
  match: TournamentMatch8;
  x: number;
  y: number;
...
export default function BracketMatchCard8({ match, x, y, teams, format, matchLabel }: Props) {
  ...
  const isGrandFinal = match.id === 'M16';
  const slotH = MATCH_H8 / 2;
  ...
  const renderSlot = (
    ...
    return (
      <g key={isTop ? 'slot1' : 'slot2'} transform={`translate(0, ${slotY})`}>
        <rect x={0} y={0} width={MATCH_W8} height={slotH} fill={getSlotFill()} />
        {seed !== null && (
        ...
        {team && (
          <text x={MATCH_W8 - 12} y={slotH / 2 + 4} textAnchor="middle"
            fill={match.status === 'completed' && isWinner ? '#bbf7d0' : match.status === 'completed' && !isWinner ? '#fca5a5' : '#fff'}
        ...
        {isTop && (
          <line x1={0} y1={slotH} x2={MATCH_W8} y2={slotH}
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      ...
      {matchLabel && (
        <text x={MATCH_W8 / 2} y={-12} textAnchor="middle"
      ...
      <rect x={-30} y={MATCH_H8 / 2 - 10} width={26} height={20} rx={3}
        fill={isGrandFinal ? '#b45309' : isBo5 ? '#92400e' : '#7f1d1d'}
      />
      <text x={-17} y={MATCH_H8 / 2 + 4} textAnchor="middle"
      ...
      <rect x={-30} y={MATCH_H8 / 2 + 12} width={26} height={12} rx={3}
        fill={isBo5 ? '#d97706' : '#292929'}
      />
      <text x={-17} y={MATCH_H8 / 2 + 21} textAnchor="middle"
      ...
      <rect x={0} y={0} width={MATCH_W8} height={MATCH_H8} rx={4}
        fill="none" stroke={borderColor} strokeWidth={isBo5 ? 2 : 1}
