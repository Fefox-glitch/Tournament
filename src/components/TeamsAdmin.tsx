...
import { Plus, Trash2, Edit3, Check, X, Users, Flag } from 'lucide-react';
...
export default function TeamsAdmin({ teams, onTeamsChange }: Props) {
  ...
  async function saveEdit() {
    if (!editDraft || !editDraft.name.trim()) { setError('El nombre del equipo es requerido.'); return; }
    setSaving(true);
    setError(null);
  ...
  const TeamForm = ({
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 rounded-xl bg-gray-950 border border-red-900/40">
      ...
      <div>
        <label className="block text-xs text-gray-400 mb-1">Seed (1–12)</label>
        <input
          type="number"
          min={1}
          max={12}
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Equipos Participantes</h2>
            <p className="text-sm text-gray-500">{teams.length}/12 equipos registrados</p>
          </div>
        </div>
        {!adding && (
          <button
      ...
      <div className="mb-4 p-3 rounded-lg bg-red-950/30 border border-red-900/30 flex items-start gap-2">
        ...
        <p className="text-xs text-gray-400">
          Asigna seeds 1–12 a tus equipos. Se colocarán automáticamente en el bracket: Seeds 1–4 entran en Ronda 2; Seeds 5–12 comienzan desde Ronda 1.
        </p>
      </div>
      ...
      <div className="space-y-2">
        {sortedTeams.length === 0 && (
          <div className="py-16 text-center text-gray-500">
            ...
            <p>Sin equipos. Agrega hasta 12 equipos para poblar el bracket.</p>
          </div>
        )}
        ...
        {sortedTeams.map(team => (
          <div key={team.id}>
            {editing === team.id && editDraft ? (
              <TeamForm
                value={editDraft}
                onChange={v => setEditDraft({ ...editDraft, ...v })}
                onSave={saveEdit}
                onCancel={cancelEdit}
              />
            ) : (
          ...
          <div className="grid grid-cols-12 gap-1">
            {Array.from({ length: 12 }, (_, i) => i + 1).map(seed => {
              ...
              return (
                <div
                  key={seed}
                  title={assigned ? assigned.name : `Seed ${seed} — empty`}
                  className={`
                    h-8 rounded flex items-center justify-center text-xs font-bold transition-colors
